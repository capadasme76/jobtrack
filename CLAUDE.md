...
**Sections**: Hoy (stats + dispatches ticker — `state.dispatches` is real & persisted, seeded once with 2 examples, not a hardcoded constant), Mi perfil (collapsed by default, `jt-collapsible-title` pattern; sub-sections Experiencia/Formación/Carta also collapse individually via `profileExpanded`), Búsquedas de empleo (always-visible, see "Watched searches" below), Pipeline (kanban board, drag/drop by stage, columns cap at `max-height:600px` with their own scroll on desktop only — see "Known-tricky areas", Excel import/export via SheetJS CDN, moved below the board as a secondary action), Match CV (`runMatchAnalysis` — client-side text/keyword matching, no external API — the result includes an explicit disclaimer that it's not a real fit evaluation), Reuniones, Mis contactos (own Excel template/import, plus a name/company search box), Métricas (didactic: % of total per stage, color-coded, tooltips), Portales (static external job-board links), Guía (static how-to reference, 9 numbered steps matching the section order above, each linking to its section).
...
**Watched searches**: same mechanism, extended to portal keyword searches. "Búsquedas de empleo" (its own top-level section, not nested in Mi perfil) writes to `state.watchedSearches` (independent of any opportunity) — typing a cargo/keyword auto-generates a search URL for ChileTrabajos (`chiletrabajos.cl/encuentra-un-empleo?...`); users can also paste an arbitrary search URL manually. Validated the rest of the `Portales` list this doesn't cover: LinkedIn is ToS-prohibited (as above), Indeed actively 403s automated requests even with a browser-like User-Agent, Laborum/Trabajando.cl are client-rendered SPAs that return an empty shell to a plain fetch, and **Computrabajo returns real content to a request from a local machine but 403s the same request from Vercel's or GitHub Actions' IP ranges** — confirmed live in production via the manual-check button below, not just guessed — so it's deliberately left out of `WATCHED_SEARCH_PORTALS` despite looking viable in earlier ad-hoc testing. `check-watched-pages.mjs` hashes these the same way as `watchedPages` and pushes the same kind of "verify" dispatch. There's also a manual per-search trigger — `api/check-watched-search.js`, called from a "Comprobar ahora" button next to each watched search — so users aren't stuck waiting for the once-a-day cron; it authenticates with the user's own access token (not service_role) and reads/writes only their own `jobtrack_state` row under RLS. An email alert on top of the in-app dispatch (via Resend) is planned but not built — would need a `RESEND_API_KEY` GitHub Actions secret and a Supabase auth-admin lookup to resolve each user's email from their `user_id`.
