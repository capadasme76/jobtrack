...
**Watched searches**: same mechanism, extended to portal keyword searches. "Mi perfil → Búsquedas vigiladas" writes to `state.watchedSearches` (independent of any opportunity) — typing a cargo/keyword auto-generates search URLs for the two portals confirmed to serve real server-rendered results to a plain fetch (`cl.computrabajo.com/trabajo-de-{slug}`, `chiletrabajos.cl/encuentra-un-empleo?...`); users can also paste an arbitrary search URL manually. Validated the rest of the `Portales` list this doesn't cover: LinkedIn is ToS-prohibited (as above), Indeed actively 403s automated requests even with a browser-like User-Agent, and Laborum/Trabajando.cl are client-rendered SPAs that return an empty shell to a plain fetch — so only Computrabajo and ChileTrabajos are wired up for now. `check-watched-pages.mjs` hashes these the same way as `watchedPages` and pushes the same kind of "verify" dispatch. An email alert on top of the in-app dispatch (via Resend) is planned but not built — would need a `RESEND_API_KEY` GitHub Actions secret and a Supabase auth-admin lookup to resolve each user's email from their `user_id`.
